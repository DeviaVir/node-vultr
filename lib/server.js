 ...
 *                       'notify_activate': {String} // 'yes' or 'no'. If yes, an activation email will be sent when the server is ready.
 *                     }
 * @return {Promise}     
 */
Server.prototype.create = function(vps) {
  ...
  if(vps.hostname !== void 0) {
    data.hostname = vps.hostname;
  }
  ...
  if(vps.notify_activate !== void 0) {
    if(vps.notify_activate !== 'yes' && vps.notify_activate !== 'no') {
      vps.notify_activate = (!!vps.notify_activate === true ? 'yes' : 'no');
    }
    data.notify_activate = vps.notify_activate;
  }
